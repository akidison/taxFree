...
class CalculateViewController: UIViewController, UITextFieldDelegate {
    ...
    func reverseCalculateValueWithDifferentValue() {
        if choosedCurrency == "RUB" {
            ...
            self.resultLabel.text = String(format: "%.3f", (Double(textViewString)!*((1/self.coefficientValue)*choosedValue)))
        }
    }
    ...
    func calculateValueWithDifferentValue() {
        if choosedCurrency == "RUB" {
            ...
            self.resultLabel.text = String(format: "%.3f", (Double(doubleValue)/((1/self.coefficientValue)*choosedValue)))
        }
    }
    ...
    @objc func addTapped() {
        ...
        if (Utils().getValueFromDefaults(key: GlobalKeys.JSON_SAVED_ARRAY) as? [String]) != nil {
            savedArrayCurrency = Utils().getValueFromDefaults(key: GlobalKeys.JSON_SAVED_ARRAY) as! [String]
        }
        if savedArrayCurrency.contains(where: {$0 == self.choosedCurrency}) {
            Utils().showToastWithCustomtexts(title: "always_added_title", message: "always_added_message", selfView: self.view, imageName: "importante.png", duration: 0.8)
        } else {
            savedArrayCurrency.append(choosedCurrency)
            Utils().saveValueForDefaults(value: savedArrayCurrency, key: GlobalKeys.JSON_SAVED_ARRAY)
