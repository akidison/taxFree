...
class CountriesRulesViewController: UITableViewController, UISearchBarDelegate {
    ...
    var countriesArray = ["Austria", "Czech_Republic", "Denmark", "Estonia", "Finland", "France", "Germany", "Greece", "Hungary", "Iceland", "Ireland", "Italy"]
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let image = UIImage(named: "\(countriesArray[indexPath.row])")
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        choosedCell = countriesArray[indexPath.row]
        self.performSegue(withIdentifier: "to_info_vc", sender: self)
    }
    
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        countriesArray = searchText.isEmpty ? countriesArray : countriesArray.filter { (item: String) -> Bool in
