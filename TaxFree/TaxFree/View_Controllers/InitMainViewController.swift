...
class InitMainViewController: UIViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        self.getJsonData(urlAddress: Domain.JSON_DATA_URL, result: { finished, error in
...
//        if (Utils().getValueFromDefaults(key: GlobalKeys.JSON_OBJECT) == nil) {
            ...
            URLSession.shared.dataTask(with: url, completionHandler: { (data, response, error) -> Void in
                if error == nil && data != nil {
                    do {
                        ...
                        print(json)
                        result(true, error)
                        Utils().saveValueForDefaults(value: json, key: GlobalKeys.JSON_OBJECT)
